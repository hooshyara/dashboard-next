...
const PRIORITY_OPTIONS = [
  ...
  { value: '5', label: 'اولویت ۵' },
];

export function DriversFilter({ filters, onChange, onFilter, onClear, className }: DriversFilterProps) {
  const hasFilters =
    filters.name ||
    filters.plateNumber ||
  ...
  return (
    <div className={`bg-card border border-border rounded-lg flex flex-col items-start p-4 mb-4 ${className}`}>
      <div className='w-full grid md:grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-4'>
        {/* Driver Name Filter */}
        ...
        {/* Priority Filter */}
        <div className='grid gap-2'>
          <Label className='text-foreground text-sm'>اولویت</Label>
          <Select
...
const DriversFilterContainer = (props: DriversFilterProps) => {
  if (props?.isDrawer) {
    return (
      <Drawer>
        <DrawerTrigger
  ...
  return <DriversFilter {...props} />;
};

export default DriversFilterContainer;
