      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{route.driver.car}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            <div className="space-y-2 min-h-[60px]">
              {orders.length === 0 ? (
                <div className="flex items-center justify-center py-8 border-2 border-dashed border-border rounded-lg text-muted-foreground text-sm">
                  سفارش را به اینجا بکشید
                </div>
              ) : (
                orders.map((order, index) => (
                  <DraggableOrder key={order.id} order={order} index={index} showDetails />
                ))
              )}
            </div>
          </SortableContext>
