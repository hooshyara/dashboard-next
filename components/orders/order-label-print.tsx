...
export function OrderLabelPrint({ order, meta, mode }: OrderLabelPrintProps) {
  ...
  const address = meta?.isMiscellaneous && meta.miscAddress ? meta.miscAddress : order.address;
  const customerName = meta?.receiverName || order.contactPerson || meta?.placerName || '-';
  const deliveryDate = order.deliveryTime ? format(new Date(order.deliveryTime), 'yyyy/MM/dd HH:mm') : '-';
    ...
    <div className="order-print-container hidden print:block bg-white text-black p-8">
      {mode === 'label' ? (
        <div className="border-2 border-black p-6 max-w-md mx-auto">
          <h1 className="text-xl font-bold text-center border-b-2 border-black pb-3 mb-4">برچسب ارسال</h1>
          <div className="space-y-3 text-base">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="leading-relaxed">{order.locationName ? `${order.locationName} — ` : ''}{address}</span>
            </div>
            {order.mobile && (
              <div className="flex justify-between gap-4">
        ...
        <div className="border-2 border-black p-6 max-w-md mx-auto">
          <h1 className="text-xl font-bold text-center border-b-2 border-black pb-3 mb-4">رسید سفارش</h1>
          <div className="space-y-3 text-base">
            <div className="flex justify-between gap-4">
            ...
            <div className="flex justify-between gap-4 border-t border-black pt-2">
              ...
              <span>
                {order.price != null ? `${Number(order.price).toLocaleString('fa-IR')} ریال` : '-'}
              </span>
            </div>
            <div className="flex justify-between gap-4">
            ...
            <div className="flex justify-between gap-4">
              ...
              <span>{meta?.paymentTime ? format(new Date(meta.paymentTime), 'yyyy/MM/dd HH:mm') : '-'}</span>
            </div>
            <div className="flex justify-between gap-4 border-t border-black pt-2">
              <span className="font-bold">کد پیگیری:</span>
              <span className="font-mono">{order.trackingCode}</span>
          ...
          <p className="text-center text-xs mt-6 pt-3 border-t border-black">با تشکر از خرید شما</p>
        </div>
      )}
    </div>
  );
