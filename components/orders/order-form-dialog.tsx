...
export function OrderFormDialog({
  ...
  useEffect(() => {
    if (order) {
      const pickupLoc = order.pickupPlaceId != null ? locations.find((l) => l.id === order.pickupPlaceId) : null;
      const dropById = order.dropoffPlaceId != null ? locations.find((l) => l.id === order.dropoffPlaceId) : null;
      const legacyDrop =
        dropById ?? locations.find((loc) => loc.title === order.locationName || loc.address === order.address);
    ...
    <>
      ...
      >
        <DialogContent className='sm:max-w-[600px] bg-card border-border max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            <DialogTitle className='text-foreground'>{order ? 'ویرایش سفارش' : 'افزودن سفارش جدید'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit}>
            <div className='grid gap-8 py-4'>
              <div className='text-sm font-medium text-foreground border-b border-border pb-2'>فیلدهای ضروری</div>
              <div className='grid gap-2'>
                ...
                {formData.pickupAddress && <p className='text-xs  mt-1'>{formData.pickupAddress}</p>}
              </div>
              <div className='grid gap-2'>
                <Label
                ...
                {formData.dropoffAddress && <p className='text-xs  mt-1'>{formData.dropoffAddress}</p>}
              </div>
                ...
                <div className='grid gap-6 grid-cols-1'>
                  <div>
                    ...
                    <PersianDatePicker
                      ...
                      onChange={(date) => setFormData({ ...formData, deliveryTime: date || null })}
                  ...
                  <div>
                    ...
                    <PersianDatePicker
                      ...
                      onChange={(date) => setFormData({ ...formData, returnTime: date || null })}
                      placeholder='انتخاب تاریخ و ساعت'
                      showTimePicker
                    />
                  </div>
              ...
              <div className='grid gap-4'>
                ...
                <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                  <div className='grid gap-2'>
                    ...
                    <Input
                      id='receiverName'
                      value={meta.receiverName}
                      onChange={(e) => setMeta({ ...meta, receiverName: e.target.value })}
                      placeholder='اختیاری'
                      className='bg-secondary border-border text-foreground'
                    />
                  </div>
                  <div className='grid gap-2'>
                    ...
                    <Input
                      id='receiverMobile'
                      dir='ltr'
                      value={meta.receiverMobile}
                      onChange={(e) => setMeta({ ...meta, receiverMobile: e.target.value })}
                      placeholder='اختیاری'
                      className='bg-secondary border-border text-foreground'
                    />
                  </div>
