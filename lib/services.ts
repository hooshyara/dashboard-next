      ...
      return assigned.map((row, routeIndex) => {
        ...
        const twStart =
          matched.length > 0
            ? new Date(
                Math.min(
                  ...matched.map((o) => new Date(o.deliveryTime).getTime()),
                ),
              )
            : range.start;
        const twEnd =
...
export async function getUsers(): Promise<User[]> {
  ...
  return await fetch(`${process.env.NEXT_PUBLIC_REDIRECT_URL}/users`).then(res => res.json());
...
export async function deleteUser(id: number) {
  // TODO: Replace with actual API call
  return fetch(`${process.env.NEXT_PUBLIC_REDIRECT_URL}/users/${id}`, { method: 'DELETE' });
}
