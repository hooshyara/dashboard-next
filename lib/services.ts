...
import { getAuthUserId, isAdminUser } from './auth';
...
function appendOrderFilterQuery(
  ...
  const d = filters.destination?.trim();
...
export async function filterDrivers(
  ...
  const res = await fetch(
    `${BASE_URL}/drivers/filter?${params.toString()}`,
    {
      method: 'GET',
      headers: {
...
export async function createDriver(driver: Omit<Driver, 'id'>): Promise<Driver> {
  ...
  const res = await fetch(`${BASE_URL}/drivers/`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-permissions': 'driver:create' },
...
export async function getOrders(): Promise<Order[]> {
  const res = await fetch(`${BASE_URL}/orders`, {
    headers: { 'Content-Type': 'application/json', 'x-permissions': 'order:read' },
  });
  if (!res.ok) {
...
export async function createOrder(
  ...
  const res = await fetch(`${BASE_URL}/orders`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-permissions': 'order:create' },
...
export async function deleteOrder(id: number): Promise<void> {
  const res = await fetch(`${BASE_URL}/orders/${id}`, {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json', 'x-permissions': 'order:delete' },
...
/** فقط سفارش‌های «تخصیص داده نشده» در بهینه‌ساز و لیست ارسال شرکت می‌کنند. */
export function ordersEligibleForOptimizer(orders: Order[]): Order[] {
  ...
  if (body && typeof body === 'object') {
    ...
    if (Array.isArray(assigned) && assigned.length > 0) {
      ...
      return assigned.map((row, routeIndex) => {
        ...
        const matched: Order[] = [];
        for (const opt of optOrders) {
          if (!opt || typeof opt !== 'object') continue;
          const ord = pickOrderMatchingOptimizerSlice(
            opt as Record<string, unknown>,
            ordersInRange,
            used
          );
          if (ord) matched.push(ord);
        }

        const resolved =
          drivers.find((d) => d.name.trim() === driverName) ??
          ({
            id: 0,
            name: driverName || '—',
            car: '',
        ...
        const twStart =
          matched.length > 0
            ? new Date(
                Math.min(...matched.map((o) => new Date(o.deliveryTime).getTime()))
              )
            : range.start;
        const twEnd =
          matched.length > 0
            ? new Date(
                Math.max(...matched.map((o) => new Date(o.deliveryTime).getTime()))
              )
            : range.end;

        const orders: Order[] = matched.map((o) => ({
          ...o,
          driverId: driver.id,
          driver,
        }));
...
export async function getDeliveryRoutes(
  ...
  const [drivers, orderRows, res] = await Promise.all([
    getActiveDrivers(),
    fetchOrdersByTimeRange(range.start, range.end),
    fetch(`${BASE_URL}/optimizer`, {
      method: 'POST',
  ...
  const body: unknown = await res.json();
  const sorted = ordersEligibleForOptimizer(orderRows).sort(
    (a, b) =>
      new Date(a.deliveryTime).getTime() - new Date(b.deliveryTime).getTime()
  );
  const routes = buildDeliveryRoutesFromOptimizerResponse(
    body,
    sorted,
    drivers,
    range
...
export function buildDeliveryRouteAssignmentPayload(
  ...
  const assignments = routes.map((r) => {
    const driverId = Number(r.driver?.id);
...
export async function createUser(user: Omit<User, 'id' | 'createdAt' | 'updatedAt'> & { password: string }): Promise<User> {
  // TODO: Replace with actual API call
...
export async function updateUser(id: number, user: Partial<User>): Promise<User> {
  ...
  return Promise.resolve({ ...existingUser, ...user, updatedAt: new Date() } as User);
}
...
export async function toggleUserStatus(id: number): Promise<User> {
  ...
  return Promise.resolve({ ...user, status: newStatus, updatedAt: new Date() } as User);
}
...
export async function updateLocation(id: number, location: Partial<Location>): Promise<Location> {
  const payload: Record<string, unknown> = {};
  if (location.title !== undefined) payload.title = location.title;
