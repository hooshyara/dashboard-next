...
async function headers(permission: Permission): Promise<HeadersInit> {
  console.log("permission: ", permission);
  await permissionStore.waitUntilReady();
  return {
    "Content-Type": "application/json",
    ...(permissionStore.has(permission) ? { "x-permissions": permission } : {}),
  };
}

function buildDriverFilterBody(
...
function appendOrderFilterQuery(
  ...
  if (filters.driverId !== null)
    params.append("driverId", String(filters.driverId));
...
function mapOrderPartialToApi(order: Partial<Order>): Record<string, unknown> {
  ...
  console.log("contactPerson: ", order.contactPerson);
  if (order.sender !== undefined) p.sender = order.sender;
  console.log('order.sender: ', order.sender);
  if (order.sender_mobile !== undefined) p.sender_mobile = order.sender_mobile;
  if (order.deliveryTime !== undefined)
    p.deliveryTime = iso(order.deliveryTime);
  if (order.description !== undefined) p.description = order.description;
  if (order.price !== undefined) p.price = order.price;
  if (order.paymentMethod !== undefined) p.paymentMethod = order.paymentMethod;
  if (order.paymentTime !== undefined) p.paymentTime = iso(order.paymentTime);
...
export async function filterDrivers(
  ...
  const res = await fetch(`${BASE_URL}/drivers/filter?${params.toString()}`, {
    method: "GET",
    headers: await headers("driver:read"),
  });
...
export async function createDriver(
  ...
  const res = await fetch(`${BASE_URL}/drivers/`, {
    method: "POST",
    headers: await headers("driver:create"),
...
export async function updateDriver(
  ...
  const res = await fetch(`${BASE_URL}/drivers/${id}`, {
    method: "PUT",
    headers: await headers("driver:update"),
...
export async function deleteDriver(id: number): Promise<void> {
  const res = await fetch(`${BASE_URL}/drivers/${id}`, {
    method: "DELETE",
    headers: await headers("driver:delete"),
  });
...
export async function filterOrders(
  ...
  const url = qs
    ? `${BASE_URL}/orders/filter?${qs}`
    : `${BASE_URL}/orders/filter`;
  const res = await fetch(url, {
    method: "GET",
    headers: await headers("order:filter"),
  });
  if (!res.ok) {
...
export async function getOrders(): Promise<Order[]> {
  const res = await fetch(`${BASE_URL}/orders`, {
    headers: await headers("order:read"),
  });
  if (!res.ok) {
...
export async function updateOrder(
  ...
  const res = await fetch(`${BASE_URL}/orders/${id}`, {
    method: "PUT",
    headers: await headers("order:update"),
    body: JSON.stringify(patch),
  });
  if (!res.ok) {
...
export async function deleteOrder(id: number): Promise<void> {
  const res = await fetch(`${BASE_URL}/orders/${id}`, {
    method: "DELETE",
    headers: await headers("order:delete"),
...
export async function fetchOrdersByTimeRange(
  start: Date,
  end: Date,
): Promise<Order[]> {
...
export async function getOrdersByTimeRange(
...
function pickOrderMatchingOptimizerSlice(
  ...
  const strict = available.find((ord) => {
    ...
    if (addr && ord.address?.trim() && ord.address.trim() !== addr)
      return false;
    return true;
  });
  if (strict) {
  ...
  const loose = available.find(
    (ord) =>
      orderDeliveryIso(ord) === deliveryTime &&
      (ord.contactPerson || "").trim() === contact,
  );
  if (loose) {
    used.add(loose.id);
    return loose;
...
function buildDeliveryRoutesFromOptimizerResponse(
  ...
  if (body && typeof body === "object") {
    ...
    if (Array.isArray(assigned) && assigned.length > 0) {
      return assigned.map((row, routeIndex) => {
        ...
        const optOrders = Array.isArray(a.assignedOrders)
          ? a.assignedOrders
          : [];
        ...
        const resolved =
          ...
          ({
            ...
            priority: Math.min(
              5,
              Math.max(1, Math.round(Number(a.priority ?? 1))),
            ),
            description: "",
            isActive: true,
          } as Driver);
        ...
        const driver: Driver = {
          ...
          priority: Math.min(
            5,
            Math.max(1, Math.round(Number(a.priority ?? resolved.priority))),
          ),
        };
        ...
        const twStart =
          optOrders.length > 0
            ? new Date(
                Math.min(
                  ...optOrders.map((o) => new Date(o.deliveryTime).getTime()),
                ),
              )
            : range.start;
        const twEnd =
  ...
  const rows = unwrapArray<unknown>(body, [
    "data",
    "routes",
    "items",
    "deliveryRoutes",
...
export async function getDeliveryRoutes(
  ...
  const res = await fetch(
    `${BASE_URL}${!isNeshanOptimizer ? "/optimizer/time" : "/optimizer/preview"}`,
    {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        ...(isNeshanOptimizer
          ? { lat: DEFAULT_DRIVER_LAT, lng: DEFAULT_DRIVER_LNG }
          : {}),
        ...(tripType ? { tripType } : {}),
      }),
    },
...
export function buildDeliveryRouteAssignmentPayload(
  routes: DeliveryRoute[],
): DeliveryRouteAssignmentPayload {
  const assignments = routes.map((r) => {
    const driverId = Number(r.driver?.driverId);
    const orderIds = r.orders
      .map((o) => Number(o.id))
      .filter((id) => Number.isFinite(id) && id > 0);
...
export function deliveryRouteAssignmentsEqual(
  ...
  for (let i = 0; i < na.length; i++) {
    ...
    const rb = nb[i];
    if (
      ra.driverId !== rb.driverId ||
      ra.orderIds.length !== rb.orderIds.length
    )
      return false;
...
export async function saveDeliveryRouteAssignments(
  payload: DeliveryRouteAssignmentPayload,
): Promise<void> {
  const headers = {
    "Content-Type": "application/json",
...
export async function getUserPermissions(id: number) {
  return await fetch(`${BASE_URL}/users/permissions/${id}`).then((res) =>
    res.json(),
  );
}
...
export async function createLocation(
  ...
  const res = await fetch(`${BASE_URL}/places/`, {
    method: "POST",
    headers: await headers("place:create"),
    body: JSON.stringify({
      name: location.name,
...
export async function updateLocation(
  ...
  const res = await fetch(`${BASE_URL}/places/${id}`, {
    method: "PUT",
    headers: await headers("place:update"),
    body: JSON.stringify(payload),
  });
  if (!res.ok) {
...
export async function deleteLocation(id: number): Promise<void> {
  ...
  const res = await fetch(`${BASE_URL}/places/${id}`, {
    method: "DELETE",
    headers: await headers("place:delete"),
