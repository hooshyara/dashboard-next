...
export const PERMISSION_NAME_TO_ID: Record<string, number> = Object.entries(PERMISSION_ID_TO_NAME).reduce(
  (acc, [id, name]) => {
    acc[name] = Number(id);
    return acc;
  },
...
export function normalizePermissions(raw: unknown): Set<Permission> {
  ...
  const pushFromValue = (value: unknown) => {
    ...
    if (typeof value === 'object') {
      ...
      const name = obj.name ?? obj.permission ?? obj.slug ?? obj.key ?? obj.title;
      if (typeof id === 'number' && PERMISSION_ID_TO_NAME[id]) {
        result.add(PERMISSION_ID_TO_NAME[id]);
        return;
      }
      if (typeof id === 'string' && /^\d+$/.test(id) && PERMISSION_ID_TO_NAME[Number(id)]) {
