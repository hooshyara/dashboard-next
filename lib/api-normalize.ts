...
export function normalizeDriver(raw: unknown): Driver {
  ...
  const id = num(d.id, 0);
  const active =
    d.active !== undefined ? bool(d.active, true) : bool(d.isActive, true);
  return {
    id,
    name: str(d.name, "—"),
    car: str(d.car, ""),
...
export function normalizeOrder(raw: unknown): Order {
  ...
  const pickupSnap = placeSnapshot(pickupRaw);
  const pickupPlaceId =
  ...
  let pickupPlaceName =
    o.pickupPlaceName != null ? str(o.pickupPlaceName) : null;

  if (!pickupPlaceName && pickupSnap?.title) pickupPlaceName = pickupSnap.title;
  ...
  return {
    ...
    lat,
    price: price as Order["price"],
    lng,
    pickupPlaceId,
    dropoffPlaceId,
    pickupPlaceName,
    paymentMethod: o.paymentMethod != null ? str(o.paymentMethod) : null,
