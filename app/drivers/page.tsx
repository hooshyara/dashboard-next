...
import { DriversFilter, DriversFilterValues } from '@/components/drivers/drivers-filter';
...
export default function DriversPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const data = hasDriverFilter(filters)
          ? await filterDrivers(filters)
          : await getDrivers();
  ...
  async function handleReorder(reorderedDrivers: Driver[]) {
    ...
    const updates = reorderedDrivers.filter(
      (d) => prevSnapshot.find((p) => p.id === d.id)?.priority !== d.priority
    );
    if (updates.length === 0) return;
    try {
      await Promise.all(
        updates.map((d) => updateDriver(d.id, { priority: d.priority }))
      );
    } catch (e) {
      console.error(e);
    ...
    <DashboardLayout title="رانندگان">
      <Card className="bg-card border-border">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button onClick={handleAddDriver} className="bg-primary text-primary-foreground">
        ...
        <CardContent>
          ...
          {filterActive && (
            <div className="text-sm text-muted-foreground mb-4">
              {drivers.length} راننده مطابق فیلتر
            </div>
          )}
