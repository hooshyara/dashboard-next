  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      if (appliedFilters.trackingCode) {
        if (String(order.trackingCode).toLowerCase().includes(appliedFilters.trackingCode.toLowerCase())) {
          return false;
        }
      }
          ...
          <div className='flex flex-wrap gap-2'>
            {REPORT_TYPE_OPTIONS.map((option) => (
              <Button
                ...
                className={reportType === option.value ? 'bg-primary text-primary-foreground' : 'border-border'}
              >
                {option.icon}
      ...
      <Card className='bg-card border-border'>
        <CardHeader className='flex flex-row items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            {appliedFilters && <span className='text-sm '>({getResultCount()} نتیجه)</span>}
          </div>
                  ...
                  <TableBody>
                    {filteredOrders.map((order) => (
                      <TableRow
                        ...
                        className='border-border'
                      >
                        <TableCell className='font-mono text-primary text-right'>{order.trackingCode}</TableCell>
                        <TableCell className='text-foreground text-right'>{order.locationName || '-'}</TableCell>
                        <TableCell className='text-foreground text-right'>
                          {getDisplayedOrderDriver(order)?.name || 'تخصیص داده نشده'}
                        </TableCell>
                        <TableCell className='text-foreground text-right'>{order.contactPerson}</TableCell>
                  ...
                  <TableBody>
                    {driverReportData.map((item) => (
                      ...
                      >
                        ...
                        <TableCell className='text-right text-foreground'>{item.driver.car}</TableCell>
                        ...
                        <TableCell className='text-center'>
                          <Badge className='bg-red-500/20 text-red-600 border-red-500/30'>{item.canceledOrders}</Badge>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
